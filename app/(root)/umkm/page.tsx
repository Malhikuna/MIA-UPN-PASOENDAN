...
import { CurrentFilter, UmkmItem } from "@/types/umkm";
...
export default function UmkmPage() {
  // const [filter, setFilter] = useState<"newest" | "nearby">("newest");
  const { umkmList } = useUmkm();
  const { currentSubCategories, handleReset, handleSubCategoryChange, currentFilter, handleCurrentFilterChange } =
    useUmkmLogic();
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center w-full mb-6 md:mb-8 mt-12 md:mt-12 gap-4">
        ...
        <div className="flex justify-start md:justify-between items-center gap-2 md:gap-7">
          <details ref={filterDropdownRef} className="dropdown w-30">
            ...
            value={currentFilter}
            onChange={(e) => handleCurrentFilterChange(e.target.value as CurrentFilter)}
          >

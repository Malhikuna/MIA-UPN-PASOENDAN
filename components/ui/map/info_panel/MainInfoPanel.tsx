...
const MainInfoPanel: React.FC<InfoPanelProps> = ({isShowMaximumMap, handleShowMaximumMap}) => {
  return (
    <div
      className={`flex flex-col md:gap-5 m justify-between bg-white md:border-r-2 border-primary-content px-5 pt-5 pb-2 ${isShowMaximumMap ? 'w-100 h-auto z-50' : 'relative h-auto md:h-full w-auto'}`}>
            ...
            .map((_, i) => (
              <div key={i} className="popup-header cursor-pointer flex gap-2 items-center p-2 bg-white rounded-md">
                ...
                <div>
                  ...
                  F&B •{" "}
                  <span className="text-primary-content-dark font-bold">
          Buka 08:00 WIB
        </span>
                </div>
              </div>
